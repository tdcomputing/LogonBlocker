...
namespace LogonBlocker
{
    ...
    internal class Program
    {
        ...
        {
            //read previous logs into lists
            var logs = ReadLogFile(false);
            if (!string.IsNullOrEmpty(logs))
            {
                var arr = logs.Replace("\r\n", "\n");
                windowsBlackIPs = arr.Split('\n').ToList();
            }

            logs = ReadLogFile(true);
            if (!string.IsNullOrEmpty(logs))
            {
                var arr = logs.Replace("\r\n", "\n");
                sqlBlackIPs = arr.Split('\n').ToList();
            }
        ...
        static void BlockIPs(bool isSQL)
        {
            ...
            var ips = string.Join("\r\n", isSQL ? sqlBlackIPs : windowsBlackIPs);
            WriteLogFile(isSQL, ips);
        }

        static void Add2FireWall(bool isSQL)
        {
            //Two inbound ip-blocking rules 'sqlserver_block_ip' and 'logon_block_ip' must be existed in Windows Firewall
            var name = isSQL ? "sqlserver_block_ip" : "logon_block_ip";
            var ips = string.Join(",", isSQL ? sqlBlackIPs : windowsBlackIPs);
        ...
        {
            string filePath = isSQL ? "sql.txt" : "win.txt"; // Specify the file path

            // Write the text to the file
            File.WriteAllText(filePath, data);
        }
        static string ReadLogFile(bool isSQL)
        {
            string filePath = isSQL ? "sql.txt" : "win.txt"; // Specify the file path
